...
function deriveDefaultGatewayUrl(): { pageUrl: string; effectiveUrl: string } {
  // When loaded in Electron (file: protocol), ask the desktop preload for the
  // actual gateway WebSocket URL instead of deriving it from window.location.
  if (
    location.protocol === "file:" &&
    typeof window !== "undefined" &&
    typeof (window as { openclawDesktop?: { getGatewayUrl?: () => string } }).openclawDesktop
      ?.getGatewayUrl === "function"
  ) {
    const desktopUrl = (
      window as unknown as { openclawDesktop: { getGatewayUrl: () => string } }
    ).openclawDesktop.getGatewayUrl();
    if (desktopUrl) {
      return { pageUrl: desktopUrl, effectiveUrl: desktopUrl };
    }
  }
