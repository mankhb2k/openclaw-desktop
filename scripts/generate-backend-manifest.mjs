...
const electronVersionArg = args
  .find((a) => a.startsWith("--electron-version="))
  ?.split("=")[1];
const releaseTagArg = args
  .find((a) => a.startsWith("--release-tag="))
  ?.split("=")[1];
...
// ── File paths ────────────────────────────────────────────────────────────────
const rootRuntimeTar = path.join(
  RELEASE_DIR,
  `layer-root-runtime-v${rootRuntimeVersion}.tar.gz`,
);
const openclawTar = path.join(
  RELEASE_DIR,
  `layer-openclaw-v${ocVersion}.tar.gz`,
);
...
const rootRuntimeLayer = {
  ...
  sha256:
    getSha256(rootRuntimeTar) ??
    existing.layers?.["root-runtime"]?.sha256 ??
    "",
  url: `${baseUrl}/layer-root-runtime-v${rootRuntimeVersion}.tar.gz`,
  compressedBytes:
    compressedBytes(rootRuntimeTar) ||
    existing.layers?.["root-runtime"]?.compressedBytes ||
    0,
...
const openclawLayer = {
  ...
  compressedBytes:
    compressedBytes(openclawTar) ||
    existing.layers?.openclaw?.compressedBytes ||
    0,
...
const manifest = {
  ...
  arch: "x64",
  layers: {
    "root-runtime": onlyOpenclaw
      ? (existing.layers?.["root-runtime"] ?? rootRuntimeLayer)
      : rootRuntimeLayer,
    openclaw: onlyRoot
      ? (existing.layers?.openclaw ?? openclawLayer)
      : openclawLayer,
  },
...
// ── Report ────────────────────────────────────────────────────────────────────
console.log(
  `[generate-manifest] Written: ${path.relative(ROOT, MANIFEST_PATH)}`,
);
console.log(`[generate-manifest] Release tag:    ${releaseTag}`);
console.log(`[generate-manifest] Electron:       ${electronVersion}`);
console.log(
  `[generate-manifest] openclaw layer: v${ocVersion}  sha256: ${manifest.layers.openclaw.sha256 ? manifest.layers.openclaw.sha256.slice(0, 16) + "…" : "(missing)"}`,
);
console.log(
  `[generate-manifest] root-runtime:   v${rootRuntimeVersion}  sha256: ${manifest.layers["root-runtime"].sha256 ? manifest.layers["root-runtime"].sha256.slice(0, 16) + "…" : "(missing)"}`,
);
...
if (!manifest.layers.openclaw.sha256) {
  warns.push(
    `openclaw tar.gz not found at: ${path.relative(ROOT, openclawTar)}`,
  );
  warns.push(`  → Run: npm run layer:pack-openclaw`);
}
if (!manifest.layers["root-runtime"].sha256) {
  warns.push(
    `root-runtime tar.gz not found at: ${path.relative(ROOT, rootRuntimeTar)}`,
  );
